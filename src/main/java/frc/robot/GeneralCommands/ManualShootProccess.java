...
import frc.robot.ChassiCommands.PersuitTX;
...
public class ManualShootProccess extends CommandBase {
  ...
  public void initialize() {
    ...
    Command setupBlock = new KeepTarget(angleSupplier, speedSupplier).alongWith(new PersuitTX());
    Command shootWhenOI = new PutInShooterWhenOI();

    shootGroup = shootWhenOI.deadlineWith(setupBlock);
    shootGroup.schedule();
  }
  ...
  public void end(boolean interrupted) {
    ...
    new CleanUp().schedule();//schedule a command that sets all motors to 0
  }
